...
def main() -> None:
    print("=" * 94)
    print("HAUPT-DB REIMPORT: SPIEL 71")
    print("=" * 94)
    ...
    backup_path = BACKUP_DIR / (
        "kreisligamanager_before_match_71_"
        f"{timestamp}.db"
    )
    ...
    print("Es wird ausschließlich Spiel 71 reimportiert.")
    ...
    try:
        ...
        valid_target = (
            ...
            and int(after[0]["minute"] or 0) == 65
            and int(after[0]["team_id"] or 0) == EXPECTED_TEAM_ID
            and after[0]["related_player_id"] is None
            and after[0]["first_name"] == "Levin"
            and after[0]["last_name"] == "Steffes"
        )
        ...
        if valid_target and valid_starters:
            connection.commit()
            print(
                "ERGEBNIS: SPIEL 71 REIMPORT ERFOLGREICH."
            )
            print(
                "Änderungen wurden in die Haupt-DB übernommen."
            )
            print(
                "65' SUBSTITUTION_OUT Levin Steffes "
                "mit related_player_id=NULL."
            )
        else:
            connection.rollback()
            print(
                "ERGEBNIS: SPIEL 71 REIMPORT FEHLGESCHLAGEN."
            )
            print(
