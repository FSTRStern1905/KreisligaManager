...
TEST_DB = Path("data/database/match_170_reimport_test.db")
...
def find_event(
    ...
    *,
    ...
    matches = [
        ...
        and row["code"] == code
        and int(row["minute"] or 0) == minute
        and int(row["team_id"] or 0) == team_id
        and int(row["related_player_id"] or 0) == related_player_id
    ]
...
def main() -> None:
    print("=" * 94)
    print("SPIEL 170 REIMPORT-TEST / VOLLSTÄNDIGE WECHSELPRÜFUNG")
    print("=" * 94)
    ...
    try:
        ...
        counts = count_substitutions_by_team(after)
        ...
        luca_id = int(luca["player_id"])
        aladin_id = int(aladin["player_id"])
        david_id = int(david["player_id"])
        jakob_id = int(jakob["player_id"])
        ...
        valid_counts = (
            counts.get(SG_KENN_TEAM_ID, {}).get(
                "SUBSTITUTION_IN",
                0,
            ) == 5
            and counts.get(SG_KENN_TEAM_ID, {}).get(
                "SUBSTITUTION_OUT",
                0,
            ) == 5
            and counts.get(TARFORST_TEAM_ID, {}).get(
                "SUBSTITUTION_IN",
                0,
            ) == 4
            and counts.get(TARFORST_TEAM_ID, {}).get(
        ...
        valid_links = all(
            ...
            and row["related_player_id"] is not None
            for row in after
        )
        ...
        print(
            "45' SG Kenn "
            "(Aladin Shallar IN ↔ Luca Welter OUT): "
            f"{'OK' if valid_kenn_pair else 'FEHLER'}"
        )
        print(
            "74' FSV Trier-Tarforst II "
            "(Jakob Schäfer IN ↔ David Fiegler OUT): "
        ...
        print(
            "Alle Wechsel mit player_id/team_id/related_player_id: "
        ...
        if (
            ...
            print(
                "ERGEBNIS: SPIEL 170 REIMPORT SAUBER."
            )
            print(
                "Alle Wechselpaare sind vollständig "
                "und konsistent."
            )
        else:
            connection.rollback()
            ...
            print(
                "ERGEBNIS: SPIEL 170 NOCH NICHT SAUBER."
            )
            print(
            ...
            if not valid_kenn_pair:
                print(
                    "Fehler: Wechselpaar SG Kenn "
                    "bei 45' ist nicht korrekt."
                )

            if not valid_tarforst_pair:
                print(
                    "Fehler: Wechselpaar Tarforst "
                    "bei 74' ist nicht korrekt."
                )
            ...
            raise RuntimeError(
                "Qualitätsprüfung für Spiel 170 "
                "nicht bestanden."
            )
